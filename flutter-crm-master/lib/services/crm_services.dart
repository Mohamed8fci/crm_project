...
class CrmService {
  ...
  final baseUrl = 'http://78.47.118.127/';
  Map _headers = {};
  ...
  Future<Response> userRegister(data) async {
    try {
      Response dataResponse = await networkService.post(Uri.parse(baseUrl + 'api/auth/register/'), body: data);
      return dataResponse;
    } on SocketException {
  ...
  Future<Response> userLogin(body) async {
    try {
      return await networkService.post(Uri.parse(baseUrl + 'api/auth/login/'),
          body: body);
    } on SocketException {
      throw HttpException("Network Error, check your internet");
    } on FormatException {
